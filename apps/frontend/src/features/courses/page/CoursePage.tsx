...
import { useCourses, useDeleteCourse, useRestoreCourse } from "../hooks/useCourses";
import { usePrograms } from "@/features/programs/hooks/usePrograms";
import { CourseTableView } from "../components/CourseTableView";
import { CourseGridView } from "../components/CourseGridView";
import { CourseTableSkeleton, CourseGridSkeleton } from "../components/CourseSkeletons";
import { CourseFormDialog } from "../components/CourseFormDialog";
import { ConfirmActionDialog } from "@/components/ui/confirm-action-dialog";
import type { CourseQuery, GetProgram, ICourseSelect } from "@my-app/shared";
...
export default function CoursePage() {
  ...
  const [viewMode, setViewMode] = useState<"table" | "grid">(() => {
    return (localStorage.getItem("courses_view_mode") as "table" | "grid") || "grid";
  });
  ...
  const { data: response, isLoading, isPlaceholderData } = useCourses(query);
  ...
  const programsMap = useMemo(() => {
    const map = new Map<number, GetProgram>();
    for (const p of programsList) {
      map.set(p.program.id, p);
    }
    return map;
  }, [programsList]);

  const selectedProgram = query.program_id ? programsMap.get(query.program_id) : undefined;

  const [formOpen, setFormOpen] = useState(false);
  const [courseToEdit, setCourseToEdit] = useState<ICourseSelect | null>(null);
  const [archiveTarget, setArchiveTarget] = useState<ICourseSelect | null>(null);
  const [restoreTarget, setRestoreTarget] = useState<ICourseSelect | null>(null);
  ...
  const handleArchiveConfirm = async () => {
    ...
    try {
      ...
      toast.success(`Course "${archiveTarget.initialism}" was archived.`);
      setArchiveTarget(null);
    } catch (err) {
      const apiErr = err as ApiError;
      toast.error(apiErr.message || "Failed to archive course.");
    }
  };
  ...
  const handleRestoreConfirm = async () => {
    ...
    try {
      ...
      toast.success(`Course "${restoreTarget.initialism}" has been restored.`);
      setRestoreTarget(null);
    } catch (err) {
      const apiErr = err as ApiError;
      toast.error(apiErr.message || "Failed to restore course.");
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage institutional courses across academic degree programs.
          </p>
        </div>
          ...
          {query.paginate && pagination ? (
            <span>
              ...
              <strong className="text-foreground">
                {courses.length > 0 ? (pagination.currentPage - 1) * (query.limit ?? 10) + 1 : 0}
              </strong>{" "}
              to{" "}
              <strong className="text-foreground">
                {Math.min(pagination.currentPage * (query.limit ?? 10), pagination.totalItems)}
              </strong>{" "}
              of <strong className="text-foreground">{pagination.totalItems}</strong> courses
            </span>
          ) : (
            <span>
              Showing all <strong className="text-foreground">{courses.length}</strong>{" "}
              {query.is_archived ? "archived" : "active"} course(s)
            </span>
          )}
        </div>
      ...
      <ConfirmActionDialog
        ...
        description={
          <span>
            ...
            <strong>
              {archiveTarget?.name} ({archiveTarget?.initialism})
            </strong>
            ?
          </span>
        }
      ...
      <ConfirmActionDialog
        ...
        description={
          <span>
            ...
            <strong>
              {restoreTarget?.name} ({restoreTarget?.initialism})
            </strong>
            .
          </span>
        }
