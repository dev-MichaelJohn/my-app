...
export class EmailService implements IEmailService {
  constructor() {
    if (env.BREVO_API_KEY) {
      console.log("Transactional Email Engine: Brevo HTTP API active.");
    } else {
      console.error("No Email Engine initialized. Please restart the server!");
    }
  }
  ...
  private sendViaBrevo(to: string, options: SendEmail["options"]): ResultAsync<void, AppError> {
    ...
    return ResultAsync.fromPromise(
      ...
      if (response.ok) {
        console.info(`Transactional email sent to ${to} via Brevo HTTP API`);
